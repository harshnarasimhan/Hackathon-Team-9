...
> **Cross-reference:** Chibuikeim's Q3 DLR result has since been corrected
> to a real 213.0 MVA winter rating (+1.4%, not the earlier +43.7%
> figure — see `dlr-2024-real-weather-rewrite.md`). That correction does
> **not** remove the case made below: DLR only recovers ~2% of this
> line's dispatch-down, so the 588.6 MWh/week constraint-based figure
> this section sizes against should be expected to shrink only slightly,
> not disappear. The exact re-solved number at 213 MVA is still pending
> (see `k_battery_siting_sizing_DLR-PATCHED.py`) — until that re-run
> happens, present the numbers below (the static-210 MVA case) as the
> sizing basis, not a DLR-adjusted one.
...
**Honest caveats for this slide:** WP2033 is a synthetic weather week (Ornstein–Uhlenbeck / Gaussian-field wind, not historical or forecast) — MWh figures are indicative of the mechanism, not a real-world forecast for these specific farms. Wind `marginal_cost` is **0.0** in every shipped `generators.csv` (not −1 EUR/MWh as an earlier draft of this note and the kit README stated) — costs are round-number placeholders either way, but 0.0 specifically means curtailment among same-side wind farms is **cost-neutral to the optimiser**, so the exact per-farm split within group B (why Drumkeen and Binbane specifically, not some other combination of the ten) is an LP-degenerate result — the pro-rata-by-real-curtailment allocation above is a reasonable way to break that tie, but a different solver or solver version could plausibly redistribute the split differently while landing on the same 588.6 MWh aggregate. Say the total with confidence; say the specific per-farm ranking as "this run's result," not as a uniquely determined fact. This is a DC linear approximation — no voltage, no reactive power, no N-1 — a screening result, not an operational recommendation.

**Validation gap, worth naming before the panel asks:** the siting/sizing numbers above (42.9 MW / 346.5 MWh conservative / 100.8 MWh episodic for Drumkeen; 48.4 MW / 232.6 MWh for Drumkeen+Binbane) are hand-derived from `k_dispatch_down_hourly_WP2033_north-west.csv` — reading off worst-hour and worst-episode values, not a scripted, re-solved check. No run here actually inserts a battery of the recommended size at the recommended bus (via `gridkit.add_battery()` or equivalent) and re-solves to confirm it removes the constraint-based curtailment it's sized against. That's a real gap, not just a disclaimer: it's possible the recommended size is off in either direction once the battery itself is allowed to interact with the dispatch (e.g. charging during an adjacent low-wind hour changes which hours actually bind). Recommend adding that validation script before this goes in front of a panel as a firm sizing recommendation rather than a screening estimate.
