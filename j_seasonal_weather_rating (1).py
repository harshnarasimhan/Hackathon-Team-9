...
def _resolve_data_file(basename):
    """Find a committed data file even if it went through a GitHub
    web-upload and picked up a '_1' (or '(1)', '(2)', ...) suffix - see
    Track A audit F-06 / F-13: this exact file was committed on the
    Dynamic-Line-Rating branch as 'finner_camp_historical_hourly_1.csv',
    not the plain name this module originally hard-coded, which made a
    fresh checkout fail with FileNotFoundError. Prefers the canonical
    unsuffixed name if it exists (so fixing the filename in git later
    "just works" with no code change), falls back to the known suffixed
    variants, and only then raises - with a clear message naming every
    path it tried, rather than a bare FileNotFoundError.
    """
    stem, ext = os.path.splitext(basename)
    candidates = [basename, f"{stem}_1{ext}", f"{stem} (1){ext}", f"{stem}_2{ext}"]
    for name in candidates:
        path = os.path.join(ROOT, name)
        if os.path.exists(path):
            return path
    raise SystemExit(
        f"none of the expected filenames for '{basename}' were found next "
        f"to this script (tried: {', '.join(candidates)}). If the file was "
        f"re-uploaded under a different suffix, add it to _resolve_data_file's "
        f"candidates list, or (better) rename the committed file to the "
        f"canonical unsuffixed name."
    )
...
WEATHER_CSV_FOR_LINE = {
    "5041-17010-2": _resolve_data_file("finner_camp_historical_hourly.csv"),
}
...
SOLAR_CSV_FOR_LINE = {
    "5041-17010-2": _resolve_data_file("pvgis_2024_solar.csv"),
}
