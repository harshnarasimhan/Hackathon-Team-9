...
defaults to ranking by total_overload_MWh (pass a different RANK_BY
...
#: and which season each network scenario pairs with (see
#: j_apply_seasonal_dlr.py's SEASON_SCENARIO for the same mapping).
#: Extend this dict, not the code, if another line gets a real
#: weather-derived rating of its own.
DLR_LINES = {
    "5041-17010-2": {"WP2033": "winter", "SV2033": "summer"},
}
DLR_PERCENTILE = "10"
...
def apply_dlr_ratings(network, scenario):
    ...
    rating for any line in DLR_LINES that has one for this scenario's
    paired season. Returns {line: (old_rating, new_rating)} for the
    printout / provenance record - empty if nothing applies (e.g. a
    scenario with no DLR_LINES entry)."""
    applied = {}
    for line, season_by_scenario in DLR_LINES.items():
        season = season_by_scenario.get(scenario)
        if season is None or line not in network.lines.index:
            continue
        old_rating = float(network.lines.at[line, "s_nom"])
        multiplier = weather.main(line=line, season=season, percentile=DLR_PERCENTILE)
        new_rating = old_rating * multiplier
        ...
        print(f"DLR rating applied: {line} {old_rating:.1f} -> {new_rating:.1f} MVA "
              f"(real-weather {season} p{DLR_PERCENTILE} multiplier {multiplier:.4f})")
    return applied
