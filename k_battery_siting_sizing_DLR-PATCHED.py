...
The 210 MVA static rating on 5041-17010-2 is replaced below with the real
winter-weather P10 rating, 301.9 MVA (+43.7%) - derived from 29 years of
Met Eireann hourly wind/temperature data at the nearest station (Finner
Camp) via a Hilpert-correlation thermal model, cross-validated three
independent ways. Only the "real ratings" run (`n`) is patched - the
"ratings-lifted" run (`m`, x1000) is deliberately left alone, since it's
meant to represent a fully unconstrained ceiling, not a specific DLR value.

VERIFIED (this patched script was actually re-run against the real kit,
not just reasoned through): "corrected total constraint-based MWh (week)"
drops from 588.6 to exactly 0.0. Reconciliation check confirms why - total
dispatch-down (all causes) came out at 3,072.0 MWh, and surplus-based-only
(ratings lifted) also came out at 3,072.0 MWh, so network_hour is 0.0 for
every hour, not just approximately. This means the winter constraint your
battery sizing was solving for (588.6 MWh, Drumkeen+Binbane 92.5%
coverage) doesn't just shrink under real weather, it disappears entirely
in this WP2033 week. Recommend re-running this same patched
script against SV2033 (summer) with 231.1 MVA instead of 301.9 - summer's
DLR uplift is smaller (+29.8% vs +43.7%) and should leave a real,
non-zero constraint-based number, since it only recovers ~2,100 of the
~21,800 MWh summer curtailment (see dlr-cross-team-impact.md). Framing:
DLR and the battery look complementary once you split by season, not
competing - DLR does nearly all the work in winter, the battery earns
its keep in summer.
"""
import sys
...
gridkit.set_rating(n, LINE, 301.9)  # DLR PATCH: real-weather winter P10 (was 210.0 MVA static)
gridkit.solve(n)
