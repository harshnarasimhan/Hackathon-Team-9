...
1. **Load the all-island network and apply the one real weather-derived
   ...
   script applies `gridkit.set_rating()` with that real rating (301.9 MVA,
   winter P10, versus the old static 210 MVA) **before** solving the
...
|---|---|---|---|
| 1 | 3581-89516-1 | 123.0 | 123 |
| 2 | 2781-4951-1 | 106.0 | 42 |
| 3 | 1122-11260-1 | 513.0 | 29 |
| 4 | 1122-1742-1 | 513.0 | 10 |
| 5 | 3082-3122-1 | 634.0 | 9 |
| 6 | 3691-4041-1 | 192.0 | 1 |
...
real-weather DLR rating applied (210 → 301.9 MVA), it drops out of the
national binding-lines set entirely under this scenario. This is a second,
independent confirmation of the earlier finding that winter DLR
essentially eliminates this line's own constraint.

**A smaller, expected side effect:** the composition and hour-counts of
the *other* binding lines shifted slightly versus the old static-rating
baseline (the top line, `3581-89516-1`, was 118 hours under the static
rating; it's now 123 with the DLR-adjusted rating on `5041-17010-2` in
effect). This is a genuine consequence of re-solving with a wider rating
on one line — the optimiser's dispatch shifts slightly elsewhere — not a
data error. Anyone comparing this list against an earlier static-rating
