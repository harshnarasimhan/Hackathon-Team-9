...
PRE-PATCH/HISTORICAL under the old 210 MVA static rating:
...
MWh/week. This script's constraint-based-ONLY total was 588.6 MWh/week,
PRE-PATCH/HISTORICAL under the old 210 MVA static rating. These were
consistent, not conflicting: re-running the stock method gave
total(3,660.6) - free(3,072.0) = 588.6 - an exact match to the
corrected per-generator pipeline, confirmed independently. See the
VERIFIED (real-weather DLR) note below for the current number.

VERIFIED (real-weather DLR patch applied, see dlr-patch-instructions-
harshitha.md): with the real winter (WP2033-paired) P10 rating for this
line, 301.9 MVA (was 210.0 MVA static), applied to the real-ratings run
(`n`) below, corrected total constraint-based MWh (week) drops from
588.6 to exactly 0.0 - the winter constraint this battery is sized
against doesn't just shrink, it disappears entirely for this WP2033
week. Total dispatch-down (all causes) and surplus-based-only (ratings
lifted) both come out at 3,072.0 MWh - confirmed by re-running end to
end. The `m` (ratings-lifted) run below is intentionally left alone -
it represents an unconstrained ceiling, not a specific DLR value.
...
gridkit.set_rating(n, LINE, 301.9)  # real-weather winter P10 (was 210.0 MVA static)
gridkit.solve(n)
